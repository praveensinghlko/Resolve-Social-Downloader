--[[
================================================================================
    Social Downloader for DaVinci Resolve - Windows Edition
...
local config = {
    basePath = os.getenv("USERPROFILE") .. "\\Downloads\\SocialDownloader\\",
    ytdlpPath = nil,
    ffmpegPath = nil,
    ffmpegDir = nil,
...
local function findExecutable(name)
    ...
    if handle then
        ...
        handle:close()
        if result and result ~= '' and not result:match("Could not find") then return result end
    end
    local paths = {
        os.getenv("LOCALAPPDATA") .. '\\yt-dlp\\' .. name .. '.exe',
        os.getenv("USERPROFILE") .. '\\scoop\\shims\\' .. name .. '.exe',
        ...
        ui:HGroup{
            ...
            ui:VGroup{
                ...
                ui:HGroup{
                    Weight = 0,
                    MinimumSize = {0, 22},
                    ui:Label{Text = 'Duration:', StyleSheet = [[QLabel{color:#888;font-size:11px;min-width:60px;}]]},
...
itm.lblPath.Text = config.basePath
...
local function init()
    log('===============================================================')
    log('  Social Downloader v5.1 (Windows Edition)')
    ...
    for _, p in ipairs(platforms) do
        if p.folder ~= "Other" then
            ...
            createDirectory(config.basePath .. p.folder .. "\\thumbnails")
        end
    end
    createDirectory(config.basePath .. "Other")
    createDirectory(config.basePath .. "Other\\thumbnails")
    ...
    if config.ytdlpPath then
        ...
        log('   Install: winget install yt-dlp')
    ...
    if config.ffmpegPath then
        ...
        log('   Install: winget install ffmpeg')
...
local function showHistoryWindow()
    ...
    function histWin.On.btnOpenFolder.Clicked(ev)
        os.execute('explorer "' .. config.basePath .. '"')
    end
...
function win.On.btnSetup.Clicked(ev)
    ...
    if config.ytdlpPath then
        local h = io.popen('"' .. config.ytdlpPath .. '" --version 2>nul')
    ...
    if resolve then
        ...
        log('DaVinci Resolve: Not connected')
    end
...
function win.On.btnGetInfo.Clicked(ev)
    ...
    local cmd = '"' .. config.ytdlpPath .. '" --dump-single-json --no-download --no-warnings "' .. url .. '" 2>nul'
    ...
    itm.lblThumbnail.Text = shortTitle .. '\n\n' .. formatDuration(duration) .. ' | ' .. channel

    itm.cmbPlatform.CurrentIndex = getPlatformIndex(detected)

    log('Video found!')
    log('   ' .. title)
    log('   ' .. formatDuration(duration) .. ' | ' .. channel)
    log('')
...
function win.On.btnDownload.Clicked(ev)
    ...
    if itm.chkAudio.Checked then
        ...
        if q == 0 then log('  Quality: Max Best')
        elseif q == 1 then log('  Quality: 4K')
        elseif q == 2 then log('  Quality: 1080p')
        else log('  Quality: 720p') end
    ...
    local tmpErr = os.getenv("TEMP") .. '\\social_dl_' .. os.time() .. '.txt'
    local tmpOut = os.getenv("TEMP") .. '\\social_dl_out_' .. os.time() .. '.txt'
    ...
    itm.btnDownload.Text = 'DOWNLOAD NOW'
    ...
    if success then
        ...
        for _, ext in ipairs(extensions) do
            local lsCmd = 'dir /b /od "' .. downloadFolder .. '*.' .. ext .. '" 2>nul'
            local lsH = io.popen(lsCmd)
            if lsH then
                local lastFile = nil
                for line in lsH:lines() do
                    lastFile = line
                end
                lsH:close()
                if lastFile and lastFile ~= '' then
                    file = downloadFolder .. lastFile
                    break
                end
            end
        end
        ...
        if file and file ~= '' then
            ...
            if config.rememberImport then
                if config.autoImport then
                    ...
                    if importToResolve(file) then
                        ...
                        log('Import failed - file may not be compatible')
                        setStatus('Import failed')
                    end
                else
                    log('Import skipped (remembered)')
                end
            elseif autoImport then
                ...
                if shouldImport then
                    ...
                    if importToResolve(file) then
                        ...
                        log('Import failed - try opening file manually')
                        setStatus('Import failed')
                    end
                else
                    log('Import skipped')
            ...
            log('Could not locate downloaded file')
        ...
        log('')
        log('Troubleshooting:')
        log('   1. Update yt-dlp: winget upgrade yt-dlp')
        log('   2. Ensure ffmpeg is installed: winget install ffmpeg')
